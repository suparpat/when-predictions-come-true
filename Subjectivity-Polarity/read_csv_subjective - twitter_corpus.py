...
import train_subjectivity_twitter_corpus as trainer
...
#Train the Naive Bayes classifier with twitter corpus
trainer
...
for index,tweet in enumerate(tweets):
    ...
    temp = trainer.classifier.prob_classify(trainer.word_feats(trainer.nltk.word_tokenize(tweet))).prob('sub')
...
df.to_csv('subjectivity-twittercorpus.csv',index=False)
